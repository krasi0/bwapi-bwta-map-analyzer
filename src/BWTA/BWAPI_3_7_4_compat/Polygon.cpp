...
namespace BWTA
{
  Polygon::Polygon()
  {
    ...
    /** 
     * another implementation (non-dependent on CGAL
     * adapted from http://alienryderflex.com/polygon/
     */
    bool Polygon::isInside(BWAPI::Position p) const
    {
        int i, j = (int)this->size() - 1;
        int x = p.x(), y = p.y();
        bool oddNodes = false;

        for (i = 0; i < (int)this->size(); i++) {
            if (((*this)[i].y() < y && (*this)[j].y() >= y
                    || (*this)[j].y() < y && (*this)[i].y() >= y)
                && ((*this)[i].x() <= x || (*this)[j].x() <= x)
            ) {
                oddNodes ^= ((*this)[i].x() + (y - (*this)[i].y()) 
                    / ((*this)[j].y() - (*this)[i].y()) * ((*this)[j].x() - (*this)[i].x()) < x);
            }
